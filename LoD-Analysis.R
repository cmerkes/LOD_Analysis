...
## Reorder models so they appear in order on the figures:
...
## Add model smoothers for markers with polynomial models selected:
...
## *** Skip to line 586 if using the Comparison-Data.csv file generated by the next
##   23 lines. ***
...
## All compared to Asymptotic Regression functions:
...
## All compared to Weibull Type II functions:
...
## All compared to Log-Logistic functions:
...
## All compared to Weibull Type I functions:
...
## This plot is not used in the manuscript:
COMP.DAT$Type <- factor(COMP.DAT$Type,levels=c("Weibull Type II",
...
## This plot is reported as numerical values within the manuscript text:
...
DAT3[DAT3$LOD==min(DAT3$LOD,na.rm=T),] #min LOD
DAT3[DAT3$LOD==max(DAT3$LOD,na.rm=T),] #max LOD
mean(DAT3$LOD,na.rm=T) #mean LOD
sd(DAT3$LOD,na.rm=T) #standard deviation LOD
DAT3[DAT3$LOQ==min(DAT3$LOQ),] #min LOQ
DAT3[DAT3$LOQ==max(DAT3$LOQ),] #max LOQ
mean(DAT3$LOQ) #mean LOQ
sd(DAT3$LOQ) #standard deviation LOQ
...
## For each assay (1 to 36), what is the maximum LOD obtained from any of the tested
##   model types divided by the minimum LOD obtained from any of the tested models?
##   Also, determine the distribution of that fold change (min, max, mean, sd):
...
## Get the relative drops (i.e. how much does the LOD drop going from 1 rep to 8?):
